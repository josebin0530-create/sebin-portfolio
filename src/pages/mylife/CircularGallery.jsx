...
async function resolveFont(font) {
  if (document.fonts?.load) {
    try {
      ...
      // Canvas text can still render with the browser fallback font.
    }
  }
  return font;
}
...
function createTextTexture(gl, text, font = 'bold 30px sans-serif', color = 'black') {
  ...
  const textureWidth = textWidth + padding;
  const textureHeight = textHeight + padding;
  canvas.width = Math.ceil(textureWidth * pixelRatio);
  canvas.height = Math.ceil(textureHeight * pixelRatio);
